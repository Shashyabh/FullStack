...
import com.shashyabh.electronic.store.dtos.ImageResponse;
import com.shashyabh.electronic.store.dtos.PageableResponse;
import com.shashyabh.electronic.store.dtos.UserDto;
import com.shashyabh.electronic.store.services.EmailService;
import com.shashyabh.electronic.store.services.FileService;
...
public class UserController {
    ...
    public boolean sendOTP(@PathVariable String email)
    {
        ...
        int O_Id= ThreadLocalRandom.current().nextInt(999999);

        //write code for send otp to email...


        String subject="Greeting from Your E store";
//		String msg=("Your otp is "+ otp);
        String message=""
                + "<div style='border:1px solid #e2e2e2; padding:20px'>"
                + "<h1>"
                + "Your Order is successfully placed with Id "
                + "<b>"+"O_Id"
                + "</n>"
                + "</h1> "
                + "</div>";
        String to=email;
        return this.emailService.sendEmail(subject, message, to);
    }
}
